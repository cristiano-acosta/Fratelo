	...
	<?php
		...
		$categories = get_categories($args);
			...
			foreach($categories as $category){
				/*= excluir o blog =*/
				//var_dump($category);
				/*= excluir o blog(32), janta(34), almoço(33), Sabores Especiais (27) e Sabores Tradicionais (26) =*/
				$exclude = array('32','34','33', '27', '26');
				if(!in_array($category->cat_ID,$exclude)):
	?>
				<div class="category_group">
					<div class="category_title span12">
						<?php
							echo '<h3><a href="" title="' . sprintf(__("View all posts in %s"),$category->name) . '" ' . '>' . $category->name. '</a> </h3> ';
						?>
					</div>
						...
						<?php
							...
							$items_do_cardapio = new WP_Query(array('post_type' => $post_type->name, 'order' => 'ASC','category__in' => $category->term_id));
